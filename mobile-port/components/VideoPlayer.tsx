...
const toIosPlaybackUrl = (url: string, container?: string, filename?: string) => {
  if (!url || browserFriendly(container, filename)) return url;
  const cleanUrl = url.split('?')[0];
  if (!/\/streams\/play\/[^/]+$/.test(cleanUrl)) return url;
  return `${cleanUrl}/hls/index.m3u8`;
};
...
export default function VideoPlayer({
  ...
  const [playerStatus, setPlayerStatus] = useState('Preparing player...');
  ...
  const rawSrc = playbackUrl || directUrl || '';
  const src = toIosPlaybackUrl(rawSrc, container, filename);
  const isIphoneStream = src.includes('/hls/index.m3u8');
  ...
  useEffect(() => {
    ...
    setPlayerStatus(src.includes('/hls/index.m3u8') ? 'Preparing iPhone HLS stream...' : 'Preparing player...');
    ...
    <div className="videoPlayer">
      {!playbackError ? (
        <div className="playerCanvas">
          <video
            ...
            key={src}
            autoPlay
            controls={isIphoneStream}
            playsInline
            preload="metadata"
            src={src}
            onClick={togglePlay}
            onLoadStart={() => setPlayerStatus(isIphoneStream ? 'Preparing iPhone HLS stream...' : 'Loading stream...')}
            onCanPlay={() => setPlayerStatus('Ready to play')}
            onWaiting={() => setPlayerStatus(isIphoneStream ? 'Transcoding for iPhone...' : 'Buffering...')}
            onStalled={() => setPlayerStatus('Stream stalled. Try the direct iPhone link below.')}
            onPlay={() => {
              setPlaying(true);
            ...
            onError={() => {
              setPlayerStatus('This stream could not play here.');
          ...
          <div className="mobilePlayerStatus">
            <strong>{playerStatus}</strong>
            {isIphoneStream && <span>iPhone HLS mode</span>}
          </div>
      ...
      <div className="videoMeta">
        ...
        {likelyUnsupported && !playbackError && (
          <span className="warn">iPhone HLS transcode</span>
        )}
        {isIphoneStream && (
          <a className="inlinePlayerButton" href={src} target="_blank" rel="noreferrer">
            Open iPhone stream
          </a>
        )}
        {filename && <span className="filename">{filename}</span>}
